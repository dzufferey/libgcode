...
import com.github.dzufferey.x3DomViewer.*
...
class Viewer extends AbstractMachine {
  ...
  override protected def linearMotion(x: Double, y: Double, z: Double, a: Double, b: Double, c: Double, f: Double) = {
    val x0 = getX
    val y0 = getY
    val z0 = getZ
  ...
  override protected def circularMotion(
      _x: Double,
      _y: Double,
    ...
    if (angle == 0.0) {
      ...
      assert(
        math.signum(delta) == math.signum(angle),
        s"delta: $delta, angle: $angle, x: $getX → $x, y: $getY -> $y, z: $getZ → $z, i: $i, j: $j, k: $k"
      )
      ...
      points ++= s", ${x} ${y} ${z}"
      val line =
        shape(lineSet(vertexCount := count.toString)(coordinate(point := points.toString)), lineAppearance("0 0 0"))
      toDraw += line
    }
...
object Viewer {
  ...
  def display(cmds: Seq[Command], debug: Boolean = false, conf: Config = Config) = {
