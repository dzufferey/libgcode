...
class Arc(val a: Double, val b: Double, val r: Double, val alpha: Double, val beta: Double) extends Curve[Arc] {
  ...
  /** Returns the tangents between the given point and the arc. The method first computes the tangents to the two circle
    * and then keeps the ones which falls within the arc's angles. The method return between 0 and 2 lines. When there
    * are 2 tangents, they start at (a,b) and end on the circle. When there is only 1 tangent, it has length 1 and (a,b)
    * is the midpoint.
    */
  def tangents2Point(a1: Double, b1: Double, ignoreBounds: Boolean = false, tolerance: Double = 1e-6): Seq[Line] = {
    if (compare((a, b), (a1, b1), tolerance)) {
      ...
      if (c == 0) { // 1 tangent
        ...
        val angles =
          Seq(math.acos(num0 / denom), -math.acos(num0 / denom), math.acos(num1 / denom), -math.acos(num1 / denom))
  ...
  def innerTangents(arc: Arc, ignoreBounds: Boolean = false, tolerance: Double = 1e-6): Seq[Line] = {
  ...
  /** Returns the tangents to the two arcs. The method first computes the tangents to the two full circles and then
    * keeps the ones which falls within the arc's angles. The method return between 0 and 4 lines. If the two circles
    * are equal then 0 tangents are returned.
    */
  def tangents2Arc(arc: Arc, ignoreBounds: Boolean = false, tolerance: Double = 1e-6): Seq[Line] = {
...
object Arc {
  ...
  def apply(
      a0: Double,
      b0: Double, // start point
      ca: Double,
      cb: Double, // center point
      a1: Double,
      b1: Double // end point
  ) = {
