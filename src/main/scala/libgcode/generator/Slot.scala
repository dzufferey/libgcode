...
object Slot {
  ...
  def layer(centerLine: Line,
    ...
    for (k <- 1 to nWidth) {
      ...
      if (conf.climb) {
        buffer += G(dir, conf.x(la1), conf.y(lb1), I(i*(currentRadius - stepWidth/2)), J(j*(currentRadius - stepWidth/2)))
        ...
        buffer += G(dir, conf.x(ra1), conf.y(rb1), I(-i*(currentRadius - stepWidth/2)), J(-j*(currentRadius - stepWidth/2)))
