...
object Hole {
  ...
  def apply(x: Double, y: Double, z: Double,
    ...
    ///TODO the link between the helices and spiral is broken

    def toNeutral = {
      buffer += G(1, conf.z(endC + conf.travelHeight))
    ...
    if (depthFirst) {
      ...
      while (currentHeight > endC) {
        ...
        buffer ++= Spiral.noFinishing(x2, y2, z2, radius - conf.finishingPass, insideOut)
      }
      toNeutral
    }
    //finishingPass
    if (conf.finishingPass > 0) {
      if (insideOut) {
        ...
        buffer ++= Spiral.noFinishing(x2, y2, z2, radius, true) //Spiral take cutter radius into account
      } else {
        val (x2,y2,z2) = conf.fromWorkplane(a, b, startC - conf.finishingPass)
        val (_, _, z3) = conf.fromWorkplane(a, b, endC - conf.finishingPass)
        val clockwise = conf.climb
        buffer ++= Helix(x2, y2, z2, radius - conf.endmillRadius, -conf.depthOfCut, nTurns, clockwise)
        buffer ++= Spiral.noFinishing(x2, y2, z3, radius, false)
      }
      toNeutral
    }
    buffer.toSeq
